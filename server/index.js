...
async function handleTranslate(req, res) {
  ...
  if (!input) {
    return res
      .status(400)
      .json({ error: "Please enter a description to convert." });
  }
  if (!groqApiKey) {
    return res.status(500).json({
      error:
        "Missing GROQ_API_KEY. Add it to your .env file before generating captions.",
    });
  }
  const systemPrompt = [
  ...
  try {
    ...
    if (!output) {
      return res
        .status(502)
        .json({ error: "The model did not return a caption." });
    }
